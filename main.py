...
class JarvisLive:
    ...
    def _on_text_command(self, text: str):
        ...
        if self._wake_enabled and not self._awake:
            ...
            return
        from memory.commands import handle as handle_memory_command
        local_result = handle_memory_command(text)
        if local_result is not None:
            self.ui.write_log(f"MEMORY: {local_result[:160]}")
    ...
    def _build_config(self) -> types.LiveConnectConfig:
        ...
        try:
            ...
            self._asst_name = (_cfg.get("assistant_name") or "Dobby").strip()
            _user_name = (_cfg.get("user_name") or "").strip()
        except Exception:
            self._asst_name = "Dobby"
            _user_name = ""
