...
def collect_environment() -> dict:
    ...
    info.update({
        "screen_capture": {"status": "available" if caps.get("screen") else "unavailable", "backends": ["mss"] if caps.get("screen") else []},
        "monitors": desktop.screen_info(),
        "mouse": "available" if caps.get("mouse") else "unavailable",
        "keyboard": "available" if caps.get("keyboard") else "unavailable",
        "clipboard": "available" if importlib.util.find_spec("pyperclip") else "optional/unavailable",
        "window_manager": "available" if caps.get("windows") else "unavailable",
        "application_discovery": "available",
        ...
        "memory": {"current_context":"available","conversation_storage":"available","long_term_memory":"available","retrieval":"available",
                   "task_state":"available" if TaskStateStore().status()["available"] else "unavailable","project_context":"available","path":str(memory_root())},
        "brain": {"master_provider":"gemini","providers":create_registry().list(),"usage_tracking":"available","budget_manager":"available"},
        "mcp": {"enabled":False,"providers":[],"external_capabilities":0},
        "agents": AgentRegistry().status(),
        "acquisition": {"discovery":"available","github_discovery":"available","sandbox":"workspace_only","evaluator":"available","adapter_system":"available","acquired_capabilities":0,"pending_candidates":len(AcquisitionRegistry().list())},
        "coding_agents": AgentDetector().status(),
        "coding_workspace": WorkspaceManager().status(),
        "agent_adapters": {"loaded":["aider","codex","copilot","claude_code","gemini_cli"],"available":AgentDetector().status().get("available",0),"errors":[]},
        "build_mode": {"enabled":True,"workflows":len(WorkspaceManager().list_projects()),"active_workflows":0,"repair_limit_default":3,"available_coding_agents":len(CodingAgentRegistry().available()),"projects":len(WorkspaceManager().list_projects())},
        "remote_core": {"enabled":True,"transport":"in-memory","secure_mode":"development","connected_devices":DeviceRegistry().status().get("online",0),"tls_state":"development"},
...
def run_live_validation() -> dict:
    """Run safe cross-platform observation checks; never types or clicks."""
    env=collect_environment()
    result={"environment":env,"screen":{},"windows":{},"applications":{},"input":{"status":"not_run","reason":"live validation never types or clicks automatically"}}
    result["windows"]={"status":"live_verified" if env.get("window_manager")=="available" else "unavailable","active":desktop.active_window()}
    result["applications"]={"status":"available","platform":env.get("os")}
    result["screen"]={"status":"live_verified" if env.get("monitors") else "unavailable","monitors":env.get("monitors",[])}
    result["status"]="live_verified" if env.get("os") in {"Windows","Darwin","Linux"} else "unsupported"
    return result


def format_report(data: dict) -> str:
    env = data
    lines = ["Dobby Cross-Platform Diagnostics", "", f"OS: {env.get('os')}", f"Kernel: {env.get('kernel')}",
