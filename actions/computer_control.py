...
def computer_control(
    ...
    Actions:
      ...
      media_play_pause — toggle system/media playback
      media_stop      — stop playback when supported
    ...
    try:
        ...
        # Cross-application media controls. These intentionally operate on the
        # current desktop rather than reopening a browser or searching the web.
        # Windows uses the OS media keys; Linux/macOS use their standard media
        # key names where PyAutoGUI exposes them. This gives Dobby a primitive
        # control path for an already-playing player.
        if action == "media_play_pause":
            return _press("playpause")

        if action == "media_stop":
            return _press("stop")

        if action == "volume_up":
            steps = max(1, min(20, int(params.get("amount", 1))))
        ...
        if action == "mute":
            return _press("volumemute")

        if action == "unmute":
            # Most desktop environments expose mute as a toggle, so this is
            # deliberately routed through the same media key rather than
            # pretending that an absolute unmute operation exists everywhere.
            return _press("volumemute")
...
TOOL = {
    ...
    "description": "The hands and eyes of Dobby: control the current desktop with mouse, keyboard, screenshots, visual element finding/clicking, window focus, scrolling, and media keys. Use the current screen instead of reopening or searching when the user refers to something already visible.",
    "parameters": {
        "type": "OBJECT",
        "properties": {
            "action": {
                "type": "STRING",
                "description": "type | smart_type | click | double_click | right_click | hotkey | press | scroll | move | drag | copy | paste | screenshot | wait | clear_field | focus_window | screen_find | screen_click | media_play_pause | media_stop | volume_up | volume_down | mute | unmute | random_data | user_data"
            },
            "text": {
                "type": "STRING",
