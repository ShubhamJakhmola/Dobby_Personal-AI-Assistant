...
def browser_control(
    ...
    # ── Persistent controlled navigation ────────────────────────────────────
    # A JARVIS-style multi-step task must stay inside one browser session.
    # The previous implementation opened navigation natively first and only
    # created an automation session for later clicks/types, which could split
    # one task across two browser instances.  Controlled navigation is now the
    # default.  Call with mode="native" only when the user explicitly wants
    # the OS default browser without automation.
    if action in ("go_to", "search", "new_tab"):
        mode = str(params.get("mode", "controlled")).lower().strip()
        if mode == "native":
            if action == "search":
    ...
    try:
        ...
        if action == "click":
            ...
            if coords:
                ...
                result = f"Visually clicked '{params.get('description','')}' at {coords}."
            else:
                result = f"Element not found on the current screen: '{params.get('description','')}'."
        elif action == "visual_type":
            from actions.computer_control import _screen_find, _click, _smart_type
            coords = _screen_find(params.get("description", ""))
            if coords:
                _click(coords[0], coords[1])
                result = _smart_type(params.get("text", ""), clear_first=params.get("clear_first", True))
            else:
                result = f"Element not found on the current screen: '{params.get('description','')}'."
        elif action == "observe":
            from computer.state import observe_desktop
            result = json.dumps(observe_desktop(), ensure_ascii=True)
        else:
    ...
    try:
        update_state(active_browser=browser or _registry._active_browser or get_state().active_browser)
        if action in {"go_to", "new_tab"} and params.get("url"):
            update_state(active_url=params.get("url"))
        elif action == "search":
            update_state(active_url="search:" + str(params.get("query", "")))
    except Exception:
        pass
    _log(player, result)
    return result
...
TOOL = {
    ...
    "description": "Controls the current web task. Prefer the existing page/session and current screen; do not search or reopen an already-active site unless necessary. Supports navigation, search, DOM interaction, visual mouse fallback, screenshots, tabs, and state observation.",
    "parameters": {
        "type": "OBJECT",
        "properties": {
            "action": {
                "type": "STRING",
                "description": "go_to | search | click | type | scroll | fill_form | smart_click | smart_type | visual_click | visual_type | get_text | get_url | current | observe | press | new_tab | close_tab | screenshot | back | forward | reload | switch | list_browsers | close | close_all"
            },
            "browser": {
                "type": "STRING",
