...
try:
    ...
    # Headless environments can import PyAutoGUI but fail during display setup.
    # Keep the action discoverable; actual screen actions will fail gracefully.
    _PYAUTOGUI = False
...
from computer.state import update_state, get_state
...
def _handle_play(parameters: dict, player) -> str:
    ...
    if video_url:
        ...
        _open_url(video_url)
        update_state(active_application="Chrome", active_browser="chrome", active_url=video_url, media_title=query, media_state="opening")
        # Focus the existing browser window; do not open/search again.
        if _PYAUTOGUI:
            try:
                ...
                pass
        time.sleep(1.0)
        update_state(media_state="playing")
        return f"Playing: {query} in the current browser."
...
def _media_key(key: str) -> str:
    """Send a desktop media key to the currently focused player/application."""
    if not _PYAUTOGUI:
        return "PyAutoGUI is not installed; media control is unavailable."
    try:
        pyautogui.press(key)
        return key
    except Exception as e:
...
def _handle_player_control(parameters: dict, player=None, speak=None) -> str:
    ...
    if action in keys:
        key = keys[action]
        # Player shortcuts only work when the player/browser has focus.
        if _PYAUTOGUI:
            try:
                from actions.computer_control import _focus_window
                _focus_window("Chrome")
        ...
        if result.startswith("Media key failed"):
            return result
        state = {"pause":"paused","resume":"playing","play_pause":"toggled","stop":"stopped"}.get(action)
        if state: update_state(media_state=state)
        return f"YouTube player command '{action}' sent to the current player."

    if action in {"volume_up", "volume_down"}:
        if not _PYAUTOGUI:
            return "PyAutoGUI is not installed; volume control is unavailable."
        key = "up" if action == "volume_up" else "down"
        steps = max(1, min(10, int(parameters.get("amount", 1))))
        try:
            for _ in range(steps):
                pyautogui.press(key)
            current = get_state().media_volume if get_state().media_volume is not None else 50
            update_state(media_volume=max(0,min(100,current + (steps*5 if key == "up" else -steps*5))))
            return f"YouTube volume {'increased' if key == 'up' else 'decreased'} by {steps} step(s)."
        except Exception as e:
            return f"YouTube volume control failed: {e}"

    if action in {"seek_forward", "seek_backward"}:
        key = "right" if action == "seek_forward" else "left"
        steps = max(1, min(12, int(parameters.get("amount", 1))))
        try:
            for _ in range(steps):
                pyautogui.press(key)
            return f"YouTube seek command sent ({steps} step(s))."
        except Exception as e:
            return f"YouTube seek failed: {e}"

    if action == "skip_ad":
        # Do not search or reopen YouTube. Inspect the current screen and click
        # the visible Skip Ad control if it exists.
        try:
            import pyautogui as _pg
            img = _pg.screenshot()
            # Reuse Dobby's existing vision-based screen finder.
            from actions.computer_control import _screen_find, _click
            coords = _screen_find("YouTube Skip Ad button")
            if not coords:
                return "No visible YouTube Skip Ad button was found on the current screen."
            _click(coords[0], coords[1])
            update_state(ad_detected=False)
            return f"Clicked the visible Skip Ad button at {coords}."
        except Exception as e:
            return f"Could not skip the current ad: {e}"

    return f"Unknown player control: {action}"
...
_ACTION_MAP = {
    ...
    "pause":     _handle_player_control,
    "resume":    _handle_player_control,
    "play_pause": _handle_player_control,
    "stop":      _handle_player_control,
    "volume_up": _handle_player_control,
    "volume_down": _handle_player_control,
    "mute":      _handle_player_control,
    "unmute":    _handle_player_control,
    "seek_forward": _handle_player_control,
    "seek_backward": _handle_player_control,
    "fullscreen": _handle_player_control,
    "exit_fullscreen": _handle_player_control,
    "skip_ad":   _handle_player_control,
}
...
def youtube_video(
    ...
    if handler is None:
        return (
            ...
            "Available: play, pause, resume, play_pause, stop, volume_up, volume_down, mute, unmute, seek_forward, seek_backward, fullscreen, exit_fullscreen, skip_ad, summarize, get_info, trending."
        )
...
TOOL = {
    ...
    "parameters": {
        ...
        "properties": {
            "action": {
                "type": "STRING",
                "description": "play | pause | resume | play_pause | stop | volume_up | volume_down | mute | unmute | seek_forward | seek_backward | fullscreen | exit_fullscreen | skip_ad | summarize | get_info | trending (default: play)"
            },
            "query": {
                "type": "STRING",
