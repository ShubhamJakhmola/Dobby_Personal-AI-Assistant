...
**Dobby is a local-first computer agent.** A holographic head sits at the centre of the HUD and **speaks your assistant's words with real lip-sync** — not a jaw flapping to the volume meter, but actual mouth shapes: lips closing on *m*, *b*, *p*, spreading on *i*, rounding on *u*. Brows ride the sentence, the eyes flick between fixation points, and it blinks. Turn the sound down and you can follow roughly what it just said.

## Dobby execution model

Dobby keeps Gemini as its reasoning and planning brain. Local runtime code remains the authority for policy, confirmation, execution, verification, recovery, and audit:

`request -> plan -> policy -> confirmation -> execute -> verify -> recover -> report`

Actions use the structured result contract in `core/execution.py`. A completed subprocess is not automatically a completed user goal: workflows require a verifier and report `verification_failed` when the expected state cannot be proven. Task state is persisted locally under `~/.dobby/tasks.json`, and audit records are written as redacted JSONL under `~/.dobby/audit.jsonl`.

Linux-first capabilities currently include safe argv-based terminal execution, process start/find/stop/inspection, native application launch checks, and user-level cron create/update/delete/enable/disable/list/inspect/run-now operations. Privileged or destructive terminal requests go through the local confirmation gate; Dobby never prepends `sudo`.

Reusable verification helpers cover files, processes, commands, applications, network ports, and cron entries. Recovery is bounded to configured retry limits and classifies common failures such as timeouts, missing commands, permission errors, and invalid arguments. Playwright remains optional; browser functionality is skipped clearly when it is not installed.
...
Current validation: 39 tests pass. Phase 3 adds intent parsing, registry-based capability selection, local argument validation, dry-run planning, dependency-aware execution, task resumption, legacy-action uncertainty tracking, and risk-based autonomy. Phase 4B adds environment diagnostics and a safe Linux live-validation command. Ordinary L0-L2 actions run without repeated confirmation; sensitive L3 actions remain confirmation-gated. Vision, memory redesign, coding-agent, MCP, and autonomous build workflows remain later phases.

## Computer use

Dobby's computer-use layer follows the same runtime as every other capability: `observe -> plan -> policy -> execute -> verify -> recover`. Linux adapters live under `computer/linux/` and are exposed through the action registry. Normal screen observation, application launch, window inspection, mouse, keyboard, and clipboard operations are autonomous L0-L2 actions. Sensitive keyboard or clipboard content and other consequential operations remain subject to the existing L3 confirmation gate.

Screen capture uses dynamically enumerated MSS monitors when available. Frames include dimensions, backend, timestamp, variance, near-black ratio, and explicit `invalid_frame`, `black_frame_detected`, `monitor_not_found`, `backend_unavailable`, `permission_denied`, or `capture_timeout` diagnostics. Dobby does not infer a screenshot when capture fails. Window and clipboard backends are optional and report `capability_unavailable` when Linux desktop tools are absent.

Application discovery prefers Linux `PATH` and `.desktop` entries, then verifies launched or closed processes with `psutil`. Playwright remains optional: browser automation reports `browser_automation_unavailable` when the package is not installed rather than claiming browser success. The current host is Windows, so Linux desktop integration is tested with mocks and is not claimed as hardware-verified here.

## Memory and context

Dobby keeps five separate local concepts:

- **Current context**: the small replaceable topic, project, task, device, constraints, and execution state for the active interaction.
- **Conversation history**: append-only dated JSONL records of messages. History is not automatically promoted to long-term memory.
- **Long-term memory**: structured records saved only through explicit memory operations such as `remember`/`save this`; retrieval uses deterministic keyword, topic, project, task, and recency signals.
- **Task state**: persisted workflow state from the existing task manager, kept separate from memory.
- **Project context**: project-specific workspace, repository, technology, goals, instructions, acceptance criteria, and failures.

`ContextPacket` sends only bounded current context, recent history, relevant memory, task state, and active project context to a future reasoning call. It never dumps the full memory store. Secret-like values such as API keys, passwords, tokens, private keys, credentials, and payment data are rejected from long-term memory and redacted from conversation/audit paths. Memory files are local and written with restrictive permissions where supported.
...
Underneath, Dobby knows what it is, what machine it runs on, what it can do and, importantly, **what it cannot do** — all assembled from the live system at session start rather than written into a prompt that goes stale.
...
Dobby holds a guard open across that gap, sized from the **device's own reported latency** rather than a guessed constant, so a machine with a large audio buffer gets a longer guard and one with a small buffer is not penalised. The microphone is **not muted** during it: both streams are reduced to band energies and as much of what was just played is subtracted from the microphone as fits, so only your assistant's own voice is dropped — replying the instant it stops still works.
