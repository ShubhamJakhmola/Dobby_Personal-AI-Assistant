...
class ActionRegistry:
    ...
    def metadata(self, name: str) -> dict:
        ...
        return {
            ...
            "requires_privilege": rec.requires_privilege,
            "network_access": rec.network_access,
            "reversible": rec.reversible,
            "supports_verification": rec.supports_verification,
        }

    def validate_parameters(self, name: str, parameters: dict | None) -> tuple[bool, str]:
        rec = self._actions.get(name)
        if rec is None:
            return False, f"Action '{name}' is not available."
        values = parameters or {}
        missing = [key for key in rec.parameters.get("required", []) if key not in values]
        if missing:
            return False, f"Missing required parameter(s): {', '.join(missing)}"
        properties = rec.parameters.get("properties", {})
        for key, value in values.items():
            ...
            if not valid:
                return False, f"Parameter '{key}' must be of type {expected}."
        return True, ""
    ...
    def run(self, name: str, parameters: dict, ctx: dict | None = None) -> str:
        ...
        valid, error = self.validate_parameters(name, parameters)
        if not valid:
            return f"Action '{name}' failed: [invalid_arguments] {error}"
        try:
...
def _validate(module, filename: str) -> ActionRecord:
    ...
    return ActionRecord(name=name, description=description.strip(), parameters=parameters,
                        ...
                        risk_level=str(tool.get("risk_level", "L1")),
                        requires_confirmation=bool(tool.get("requires_confirmation", False)),
                        requires_privilege=bool(tool.get("requires_privilege", False)),
                        network_access=bool(tool.get("network_access", False)),
                        reversible=bool(tool.get("reversible", True)),
                        supports_verification=bool(tool.get("supports_verification", False)))
